...
function shell(body){
  ...
  return `<div class="app-shell"><aside class="sidebar"><div class="logo-box"><img src="./logo-transparent.png?v=950" alt="수이음 로고"><div class="side-academy">${esc(state.academy?.name||'수이음학원')}</div><div class="side-sub">학원 재무를 한눈에</div></div><nav class="side-nav">${side}</nav><button class="side-logout" onclick="logout()">↪ 로그아웃</button></aside><main class="main"><div class="top"><div class="mobile-brand"><img src="./logo-transparent.png?v=950" alt="수이음"><div><div class="brand">${esc(state.academy?.name||'수이음학원')}</div><div class="muted">학원 재무를 한눈에</div></div></div><button class="btn logout" onclick="logout()">로그아웃</button></div><div class="wrap">${body}</div></main></div><div class="nav"><div class="nav-inner">${bottom}</div></div>`
}
...
function evidencePasteBox(key){
  return `<div class="paste-evidence" id="${key}PasteZone" role="group" aria-label="증빙 화면 캡처 붙여넣기" ondragover="event.preventDefault();this.classList.add('drag')" ondragleave="this.classList.remove('drag')" ondrop="handleEvidenceDrop(event,'${key}')">
    <div class="paste-evidence-empty" id="${key}PasteEmpty">
      <b>📋 화면 캡처를 바로 붙여넣기</b>
      <div class="paste-target" id="${key}PasteTarget" contenteditable="true" spellcheck="false" role="textbox" tabindex="0" aria-label="여기를 클릭하고 Ctrl V로 화면 캡처 붙여넣기" onpaste="handleEvidencePaste(event,'${key}')" oninput="captureNativePastedImage('${key}',this)" onkeydown="handlePasteTargetKey(event)">여기를 한 번 클릭한 뒤 <strong>Ctrl + V</strong></div>
      <span>캡처 도구에서 복사한 이미지를 이 박스에 붙여넣으세요.</span>
      <button type="button" class="btn paste-read-btn" onclick="readClipboardImage('${key}')">클립보드에서 붙여넣기</button>
      <small>권한 허용을 했는데 버튼이 안 될 때도 위의 흰색 붙여넣기 박스 + Ctrl+V 방식은 사용할 수 있습니다.</small>
    </div>
    <div class="paste-evidence-preview" id="${key}PastePreview" hidden><img id="${key}PasteImage" alt="붙여넣은 증빙 미리보기"><div><b>증빙 이미지가 준비되었습니다.</b><button type="button" class="text-link" onclick="event.stopPropagation();clearEvidencePaste('${key}')">붙여넣기 취소</button></div></div>
  </div>`;
}
