...
async function transactions(){const{data}=await sb.from('transactions').select('id,transaction_date,description,transaction_lines(account_id,debit,credit,accounts(id,name,type))').eq('academy_id',state.academy.id).order('transaction_date',{ascending:false}).limit(100);$('#app').innerHTML=shell(`<div class="section-title">거래 내역</div><div class="card">${data?.length?data.map(t=>`<button class="row" style="width:100%;border:0;background:transparent;text-align:left;cursor:pointer;padding-left:0;padding-right:0" onclick="transactionDetail('${t.id}')"><div><b>${t.description}</b><div class="muted">${t.transaction_date} · ${t.transaction_lines.map(l=>l.accounts?.name).join(' / ')}</div></div><span class="amount">${won(Math.max(...t.transaction_lines.map(l=>Number(l.debit||l.credit))))}</span></button>`).join(''):'<div class="empty">거래가 없습니다.</div>'}</div><div class="notice">거래를 누르면 수정하거나 삭제할 수 있습니다.</div>`)}

async function transactionDetail(id){
  const{data:t,error}=await sb.from('transactions').select('id,transaction_date,description,transaction_lines(account_id,debit,credit,accounts(id,name,type))').eq('id',id).eq('academy_id',state.academy.id).maybeSingle();
  if(error||!t){alert(error?.message||'거래를 찾을 수 없습니다.');return}
  const rev=t.transaction_lines.find(l=>l.accounts?.type==='revenue');
  const exp=t.transaction_lines.find(l=>l.accounts?.type==='expense');
  const kind=rev?'income':'expense';
  const cat=rev||exp;
  const cash=t.transaction_lines.find(l=>l.accounts?.type==='asset');
  const amt=Math.max(...t.transaction_lines.map(l=>Number(l.debit||l.credit||0)));
  const catType=kind==='income'?'revenue':'expense';
  const options=(type,selected)=>state.accounts.filter(a=>a.type===type).map(a=>`<option value="${a.id}" ${a.id===selected?'selected':''}>${a.name}</option>`).join('');
  $('#app').innerHTML=shell(`<div class="section-title">거래 수정</div><div class="card"><div class="field"><label>날짜</label><input id="editDate" type="date" value="${t.transaction_date}"></div><div class="field"><label>내용</label><input id="editDesc" value="${String(t.description||'').replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;')}"></div><div class="field"><label>금액</label><input id="editAmount" type="number" min="1" value="${amt}"></div><div class="field"><label>${kind==='income'?'수익 계정':'비용 계정'}</label><select id="editCategory">${options(catType,cat?.account_id)}</select></div><div class="field"><label>입출금 계정</label><select id="editCash">${options('asset',cash?.account_id)}</select></div><button class="btn primary" onclick="updateTx('${t.id}','${kind}')">수정 저장</button><button class="btn" style="width:100%;margin-top:10px;background:#fff0f0;color:#d93025" onclick="deleteTx('${t.id}')">이 거래 삭제</button><button class="btn" style="width:100%;margin-top:10px" onclick="go('tx')">목록으로</button><p id="msg" class="muted"></p></div>`)
}

async function updateTx(id,kind){
  const amt=Number($('#editAmount').value),desc=$('#editDesc').value.trim(),cat=$('#editCategory').value,cash=$('#editCash').value;
  if(!amt||!desc){$('#msg').textContent='내용과 금액을 입력해주세요.';return}
  const lines=kind==='income'?[{account_id:cash,debit:amt,credit:0},{account_id:cat,debit:0,credit:amt}]:[{account_id:cat,debit:amt,credit:0},{account_id:cash,debit:0,credit:amt}];
  $('#msg').textContent='수정 중…';
  const{error}=await sb.rpc('update_journal',{p_transaction:id,p_date:$('#editDate').value,p_description:desc,p_lines:lines});
  if(error){$('#msg').textContent=error.message;return}
  state.tab='tx';await transactions();
}

async function deleteTx(id){
  if(!confirm('이 거래를 삭제할까요? 삭제하면 재무제표에서도 즉시 빠집니다.'))return;
  const{error}=await sb.rpc('delete_journal',{p_transaction:id});
