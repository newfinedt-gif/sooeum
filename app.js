...
if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=945').catch(()=>{});
...
function shell(body){
  ...
  return `<div class="app-shell"><aside class="sidebar"><div class="logo-box"><img src="./logo-transparent.png?v=945" alt="수이음 로고"><div class="side-academy">${esc(state.academy?.name||'수이음학원')}</div><div class="side-sub">학원 재무를 한눈에</div></div><nav class="side-nav">${side}</nav><button class="side-logout" onclick="logout()">↪ 로그아웃</button></aside><main class="main"><div class="top"><div class="mobile-brand"><img src="./logo-transparent.png?v=945" alt="수이음"><div><div class="brand">${esc(state.academy?.name||'수이음학원')}</div><div class="muted">학원 재무를 한눈에</div></div></div><button class="btn logout" onclick="logout()">로그아웃</button></div><div class="wrap">${body}</div></main></div><div class="nav"><div class="nav-inner">${bottom}</div></div>`
}
...
async function home(){
  ...
  <div class="dash-grid"><section class="card chart-card"><h3>월별 손익 추이 (최근 6개월)</h3><div class="legend"><span class="blue">● 매출</span><span class="red">● 비용</span><span class="purple">● 순이익</span></div>${trendSvg(hist)}</section><section class="card"><h3>계좌별 잔액</h3>${key.map(a=>`<div class="row"><span>${a.name}</span><b class="${a.code==='203'?'red':''}">${won((b['code:'+a.code]??b[a.id]??0))}</b></div>`).join('')}<button class="text-link" onclick="go('funds')">자금 현황 전체보기 →</button></section></div>
...
async function funds(){const b=await balances();const key=['101','102','103','203'].map(accByCode).filter(Boolean);$('#app').innerHTML=shell(`${monthBar()}<div class="section-title">${monthTitle()} 말 자금 현황</div><div class="grid two">${key.map(a=>`<button class="card metric account-card" onclick="accountDetail('${a.id}')"><span class="muted">${a.name}</span><b class="${a.code==='203'?'red':''}">${won((b['code:'+a.code]??b[a.id]??0))}</b><small>상세내역 보기 →</small></button>`).join('')}</div><div class="section-title">카드·자금 처리</div><div class="card action-list"><button class="action" onclick="fundAction('cardDeposit')"><b>카드매출 입금</b><span class="muted">카드미수금 → 보통예금</span></button><button class="action" onclick="fundAction('cardPay')"><b>카드대금 결제</b><span class="muted">보통예금 → 카드미지급금</span></button><button class="action" onclick="fundAction('transfer')"><b>현금·계좌 자금이동</b><span class="muted">자산 계정끼리 이동</span></button></div><div class="notice">카드로 매출을 받으면 카드미수금이 늘고, 실제 카드사 입금일에 ‘카드매출 입금’을 기록하세요.</div>`)}
