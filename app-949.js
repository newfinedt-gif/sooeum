...
function shell(body){
  ...
  return `<div class="app-shell"><aside class="sidebar"><div class="logo-box"><img src="./logo-transparent.png?v=948" alt="수이음 로고"><div class="side-academy">${esc(state.academy?.name||'수이음학원')}</div><div class="side-sub">학원 재무를 한눈에</div></div><nav class="side-nav">${side}</nav><button class="side-logout" onclick="logout()">↪ 로그아웃</button></aside><main class="main"><div class="top"><div class="mobile-brand"><img src="./logo-transparent.png?v=948" alt="수이음"><div><div class="brand">${esc(state.academy?.name||'수이음학원')}</div><div class="muted">학원 재무를 한눈에</div></div></div><button class="btn logout" onclick="logout()">로그아웃</button></div><div class="wrap">${body}</div></main></div><div class="nav"><div class="nav-inner">${bottom}</div></div>`
}
...
function evidencePasteBox(key){
  return `<div class="paste-evidence" id="${key}PasteZone" tabindex="0" role="group" aria-label="증빙 화면 캡처 붙여넣기" ondragover="event.preventDefault();this.classList.add('drag')" ondragleave="this.classList.remove('drag')" ondrop="handleEvidenceDrop(event,'${key}')">
    <div class="paste-evidence-empty" id="${key}PasteEmpty"><b>📋 화면 캡처 복사 → Ctrl + V</b><span>이 페이지를 열어둔 상태에서 <strong>어디서든 Ctrl + V</strong> 하세요.</span><button type="button" class="btn paste-read-btn" onclick="readClipboardImage('${key}')">클립보드에서 붙여넣기</button><small>Ctrl+V가 안 되면 위 버튼을 눌러주세요. 처음 한 번 브라우저 권한 확인이 뜰 수 있습니다.</small></div>
