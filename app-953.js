...
function shell(body){
  ...
  return `<div class="app-shell"><aside class="sidebar"><div class="logo-box"><img src="./logo-transparent.png?v=952" alt="수이음 로고"><div class="side-academy">${esc(state.academy?.name||'수이음학원')}</div><div class="side-sub">학원 재무를 한눈에</div></div><nav class="side-nav">${side}</nav><button class="side-logout" onclick="logout()">↪ 로그아웃</button></aside><main class="main"><div class="top"><div class="mobile-brand"><img src="./logo-transparent.png?v=952" alt="수이음"><div><div class="brand">${esc(state.academy?.name||'수이음학원')}</div><div class="muted">학원 재무를 한눈에</div></div></div><button class="btn logout" onclick="logout()">로그아웃</button></div><div class="wrap">${body}</div></main></div><div class="nav"><div class="nav-inner">${bottom}</div></div>`
}
...
async function home(){
  ...
  const[{data:tx},hist,b,{data:monthLines}]=await Promise.all([
    sb.from('transactions').select('id,transaction_date,description,counterparty,memo,evidence_name,transaction_lines(debit,credit,accounts(name,type))').eq('academy_id',state.academy.id).gte('transaction_date',s).lte('transaction_date',e).order('transaction_date',{ascending:false}).limit(8),
  ...
  const normalTx=(tx||[]).filter(t=>t.description!=='초기잔액 설정'), total=normalTx.length, ev=normalTx.filter(t=>t.evidence_name).length, rate=total?Math.round(ev/total*100):0,missing=Math.max(0,total-ev);
  ...
  ${missing?`<button class="evidence-alert" onclick="evidenceManager()"><span>📎 증빙이 필요한 거래가 ${missing}건 있습니다.</span><b>확인하기 →</b></button>`:''}
  ...
  <div class="dash-grid"><section class="card"><h3>지출 카테고리 TOP 5</h3>${categoryBars(cats)}<button class="text-link" onclick="go('fs')">손익계산서 자세히 →</button></section><section class="card evidence-card"><h3>증빙 현황</h3><div class="evidence-rate">${rate}%</div><div class="progress"><i style="width:${rate}%"></i></div><div class="muted">첨부 ${ev}건 / 최근 거래 ${total}건</div><button class="text-link" onclick="evidenceManager()">증빙 관리 →</button></section></div>
  ...
  <div class="section-title">최근 거래</div><div class="card">${tx?.length?tx.map(t=>`<button class="row txrow" onclick="transactionDetail('${t.id}')"><div><b>${esc(t.description)}</b><div class="muted">${t.transaction_date} · ${txKind(t)} ${t.counterparty?'· '+esc(t.counterparty):''} ${t.evidence_name?'· 📎 증빙':''}</div></div><span class="amount ${txKindClass(t)}">${won(txAmount(t))}</span></button>`).join(''):'<div class="empty">등록된 거래가 없습니다.<br><button class="text-link" onclick="go(\'add\')">첫 거래 입력하기</button></div>'}</div>${state.install?'<button class="btn install" style="width:100%" onclick="installApp()">📲 홈 화면에 앱 설치하기</button>':''}`;
  $('#app').innerHTML=shell(body); refreshHomeBalanceCard()
}
async function transactions(){
  const[s,e]=monthRange();
  const{data}=await sb.from('transactions').select('id,transaction_date,description,counterparty,memo,evidence_name,transaction_lines(account_id,debit,credit,accounts(id,name,type))').eq('academy_id',state.academy.id).gte('transaction_date',s).lte('transaction_date',e).order('transaction_date',{ascending:false}).limit(500);
  state.txCache=data||[];
  const controls=`<div class="tx-tools"><div class="search-box"><span>⌕</span><input id="txSearch" placeholder="내용·거래처·메모 검색" oninput="filterTransactions()"></div><select id="txType" onchange="filterTransactions()"><option value="all">전체 유형</option><option value="매출">매출</option><option value="비용">비용</option><option value="자금이동">자금이동</option><option value="카드대금">카드대금</option></select><select id="txEvidence" onchange="filterTransactions()"><option value="all">증빙 전체</option><option value="yes">증빙 있음</option><option value="no">증빙 없음</option></select></div>`;
  $('#app').innerHTML=shell(`${monthBar()}<div class="section-title">${monthTitle()} 거래 내역</div>${controls}<div id="txResults"></div><div class="notice">검색·유형·증빙 여부로 거래를 빠르게 찾을 수 있습니다. 거래를 누르면 수정·삭제와 증빙 확인이 가능합니다.</div>`);
  filterTransactions();
}
function filterTransactions(){
  const q=($('#txSearch')?.value||'').trim().toLowerCase(),type=$('#txType')?.value||'all',ev=$('#txEvidence')?.value||'all';
  const list=(state.txCache||[]).filter(t=>{
    ...
    const me=ev==='all'||(ev==='yes'?!!t.evidence_name:!t.evidence_name);
    return mq&&mt&&me;
  });
...
async function transactionDetail(id){
  const{data:t,error}=await sb.from('transactions').select('id,transaction_date,description,counterparty,memo,evidence_name,evidence_data,transaction_lines(account_id,debit,credit,accounts(id,code,name,type))').eq('id',id).eq('academy_id',state.academy.id).maybeSingle();
...
async function updateTx(id,kind){
  ...
  clearEvidencePaste('editEvidence');state.tab='tx';await transactions()
}
...
function evidencePasteBox(key){
  return `<div class="paste-evidence" id="${key}PasteZone" role="group" aria-label="증빙 이미지 첨부" ondragover="event.preventDefault();this.classList.add('drag')" ondragleave="this.classList.remove('drag')" ondrop="handleEvidenceDrop(event,'${key}')">
    <div class="paste-evidence-empty" id="${key}PasteEmpty">
      <b>📎 증빙 이미지 넣기</b>
      <div class="drop-primary">
        <strong>캡처 이미지를 이곳에 끌어다 놓으세요</strong>
        <span>또는 아래 버튼에서 이미지를 선택하세요</span>
      </div>
      <label class="native-file-label" for="${key}File">이미지 선택</label>
      <input id="${key}File" class="native-file-input" type="file" accept="image/png,image/jpeg,image/webp,image/gif" onchange="handleEvidenceFile(event,'${key}')">
      <div class="paste-divider"><span>또는</span></div>
      <div class="paste-target" id="${key}PasteTarget" contenteditable="true" spellcheck="false" role="textbox" tabindex="0" aria-label="여기를 클릭하고 Ctrl V로 화면 캡처 붙여넣기" onpaste="handleEvidencePaste(event,'${key}')" oninput="captureNativePastedImage('${key}',this)" onkeydown="handlePasteTargetKey(event)">여기를 클릭한 뒤 <strong>Ctrl + V</strong></div>
      <small>Ctrl+V가 브라우저에서 막히면 위의 끌어놓기 또는 이미지 선택을 사용하세요.</small>
    </div>
    <div class="paste-evidence-preview" id="${key}PastePreview" hidden><img id="${key}PasteImage" alt="증빙 미리보기"><div><b>증빙 이미지가 준비되었습니다.</b><button type="button" class="text-link" onclick="event.stopPropagation();clearEvidencePaste('${key}')">이미지 바꾸기</button></div></div>
  </div>`;
}
function handleEvidenceFile(event,key){
  const file=event.target?.files?.[0];
  if(!file)return;
  setEvidencePaste(key,file);
}
function handlePasteTargetKey(event){
  if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==='v')return;
  if(['Tab','Shift','Control','Meta','Alt'].includes(event.key))return;
  event.preventDefault();
}

async function captureNativePastedImage(key,target){
  // Some Chrome/Edge clipboard paths insert an <img> into contenteditable
  // without exposing an image File through clipboardData. Capture that native insert.
  ...
  try{
    ...
    target.innerHTML='여기를 한 번 클릭한 뒤 <strong>Ctrl + V</strong>';
    await setEvidencePaste(key,file);
  }catch(e){
    evidenceMessage(key,'붙여넣은 캡처를 읽지 못했습니다. 캡처 후 바로 Ctrl+V를 다시 눌러주세요.',true);
    target.innerHTML='여기를 한 번 클릭한 뒤 <strong>Ctrl + V</strong>';
  }
}

function evidenceMessage(key,msg,isError=false){
  const zone=$('#'+key+'PasteZone');
  if(!zone)return;
  let el=$('#'+key+'PasteMessage');
  if(!el){el=document.createElement('div');el.id=key+'PasteMessage';el.className='paste-evidence-message';zone.appendChild(el)}
  el.textContent=msg||'';el.classList.toggle('error',!!isError);
}

async function readClipboardImage(key){
  try{
    if(!navigator.clipboard||!navigator.clipboard.read){evidenceMessage(key,'이 브라우저는 클립보드 이미지 읽기를 지원하지 않습니다. Ctrl + V를 사용해주세요.',true);return}
    const items=await navigator.clipboard.read();
    for(const clip of items){
      const type=(clip.types||[]).find(t=>t.startsWith('image/'));
      if(type){const blob=await clip.getType(type);const file=new File([blob],'screenshot_'+Date.now()+'.png',{type});await setEvidencePaste(key,file);return}
    }
    evidenceMessage(key,'클립보드에서 이미지를 찾지 못했습니다. 화면 캡처 후 다시 시도해주세요.',true);
  }catch(e){
    evidenceMessage(key,'클립보드 접근이 차단되었습니다. 주소창 옆 자물쇠 → 사이트 권한에서 클립보드를 허용하거나 Ctrl + V를 사용해주세요.',true);
  }
}
...
document.addEventListener('paste',async function(event){
  ...
  let file=files.find(x=>x.type&&x.type.startsWith('image/'));
  const items=Array.from(event.clipboardData?.items||[]);
  const item=items.find(x=>x.kind==='file'&&x.type&&x.type.startsWith('image/'));
  if(!file)file=item?.getAsFile?.()||null;
  if(!file)return;
  const zones=Array.from(document.querySelectorAll('.paste-evidence')).filter(z=>z.offsetParent!==null);
  if(!zones.length)return;
  event.preventDefault();
  const zone=zones[0];
  const key=zone.id.replace(/PasteZone$/,'');
  if(!file){evidenceMessage(key,'붙여넣은 이미지를 읽지 못했습니다.',true);return}
  await setEvidencePaste(key,file);
});

async function handleEvidencePaste(event,key){
  const files=Array.from(event.clipboardData?.files||[]);
  let file=files.find(x=>x.type&&x.type.startsWith('image/'));
  if(!file){
    const items=Array.from(event.clipboardData?.items||[]);
  ...
  // Do not prevent default here. Chrome/Edge may natively insert an <img>;
  // the contenteditable input handler will capture it.
  setTimeout(()=>captureNativePastedImage(key,event.currentTarget),0);
}
async function handleEvidenceDrop(event,key){
  event.preventDefault();event.currentTarget?.classList.remove('drag');
  const file=Array.from(event.dataTransfer?.files||[]).find(x=>x.type&&x.type.startsWith('image/'));
  if(!file){evidenceMessage(key,'이미지만 놓을 수 있습니다.',true);return}
  await setEvidencePaste(key,file);
}
async function setEvidencePaste(key,file){
  if(!file.type.startsWith('image/')){evidenceMessage(key,'이미지만 붙여넣을 수 있습니다.',true);return}
  if(evidenceDrafts[key]?.url)URL.revokeObjectURL(evidenceDrafts[key].url);
  const url=URL.createObjectURL(file);
  evidenceDrafts[key]={file,url,name:file.name||('screenshot_'+Date.now()+'.png')};
  const empty=$('#'+key+'PasteEmpty'), preview=$('#'+key+'PastePreview'), img=$('#'+key+'PasteImage');
  if(empty)empty.hidden=true;if(preview)preview.hidden=false;if(img)img.src=url;
  evidenceMessage(key,'이미지 선택 완료 ✓');
}
function clearEvidencePaste(key){
  if(evidenceDrafts[key]?.url)URL.revokeObjectURL(evidenceDrafts[key].url);
  delete evidenceDrafts[key];
  const empty=$('#'+key+'PasteEmpty'), preview=$('#'+key+'PastePreview'), img=$('#'+key+'PasteImage'),msg=$('#'+key+'PasteMessage');
  if(empty)empty.hidden=false;if(preview)preview.hidden=true;if(img)img.removeAttribute('src');if(msg)msg.remove();
}
async function compressEvidence(id){
  const pasted=evidenceDrafts[id];
  const input=$('#'+id+'File');
  const file=pasted?.file||input?.files?.[0];
  if(!file)return {data:null,name:null};
  if(!file.type.startsWith('image/'))return {error:'이미지만 첨부할 수 있습니다.'};
  try{
...
async function evidenceManager(){
  const[s,e]=monthRange();
  const{data,error}=await sb.from('transactions').select('id,transaction_date,description,counterparty,memo,evidence_name,transaction_lines(debit,credit,accounts(name,type))').eq('academy_id',state.academy.id).gte('transaction_date',s).lte('transaction_date',e).order('transaction_date',{ascending:false}).limit(500);
  if(error){alert(error.message);return}
  const list=(data||[]).filter(t=>t.description!=='초기잔액 설정'), attached=list.filter(t=>t.evidence_name), missing=list.filter(t=>!t.evidence_name);
  $('#app').innerHTML=shell(`${monthBar()}<div class="section-title">${monthTitle()} 증빙 관리</div><div class="grid two"><div class="card metric"><span class="muted">증빙 첨부</span><b class="purple">${attached.length}건</b></div><div class="card metric"><span class="muted">증빙 미첨부</span><b class="${missing.length?'red':''}">${missing.length}건</b></div></div><div class="tabs evidence-tabs"><button class="btn on" id="evMissingBtn" onclick="showEvidenceList('missing')">미첨부 ${missing.length}건</button><button class="btn" id="evAttachedBtn" onclick="showEvidenceList('attached')">첨부 ${attached.length}건</button></div><div id="evidenceList"></div>`);
  state.evidenceCache={attached,missing}; showEvidenceList('missing');
}
function showEvidenceList(mode){
  const list=state.evidenceCache?.[mode]||[];
  $('#evMissingBtn')?.classList.toggle('on',mode==='missing');
  $('#evAttachedBtn')?.classList.toggle('on',mode==='attached');
  const el=$('#evidenceList'); if(!el)return;
  el.innerHTML=`<div class="card">${list.length?list.map(t=>`<button class="row txrow" onclick="transactionDetail('${t.id}')"><div><b>${esc(t.description)}</b><div class="muted">${t.transaction_date} · ${txKind(t)} ${t.counterparty?'· '+esc(t.counterparty):''} ${t.evidence_name?'· 📎 '+esc(t.evidence_name):'· 증빙 필요'}</div></div><span class="amount ${txKindClass(t)}">${won(txAmount(t))}</span></button>`).join(''):`<div class="empty">${mode==='missing'?'모든 거래에 증빙이 첨부되어 있습니다.':'첨부된 증빙이 없습니다.'}</div>`}</div>`;
}

async function report(){
  const x=await summary(),b=await balances(),[s,e]=monthRange();
  const[{data:tx},{data:lines}]=await Promise.all([
    sb.from('transactions').select('id,transaction_date,description,counterparty,memo,evidence_name,transaction_lines(debit,credit,accounts(name,type))').eq('academy_id',state.academy.id).gte('transaction_date',s).lte('transaction_date',e).order('transaction_date',{ascending:false}).limit(300),
    sb.from('transaction_lines').select('debit,credit,accounts(name,type),transactions!inner(academy_id,transaction_date)').eq('transactions.academy_id',state.academy.id).gte('transactions.transaction_date',s).lte('transactions.transaction_date',e)
  ]);
  const reportTx=(tx||[]).filter(t=>t.description!=='초기잔액 설정'),total=reportTx.length,ev=reportTx.filter(t=>t.evidence_name).length, displayedEquity=Number(x.equity||0)+Number(x.net_income||0), margin=Number(x.revenue||0)?Math.round(Number(x.net_income||0)/Number(x.revenue||0)*1000)/10:0;
  ...
  $('#app').innerHTML=shell(`${monthBar()}<div class="report-toolbar"><div><h2>${monthTitle()} 월간 재무 보고서</h2><div class="muted">${esc(state.academy.name)} · 경영관리용</div></div><button class="btn primary" onclick="window.print()">PDF로 저장 / 인쇄</button></div><article class="print-report"><div class="report-brand"><img src="./logo-transparent.png?v=952"><div><h1>${esc(state.academy.name)}</h1><p>${monthTitle()} 월간 재무 보고서</p></div></div><div class="report-kpis"><div><span>매출</span><b>${won(x.revenue)}</b></div><div><span>비용</span><b>${won(x.expense)}</b></div><div><span>순이익</span><b>${won(x.net_income)}</b></div><div><span>자산</span><b>${won(x.assets)}</b></div><div><span>순이익률</span><b>${margin}%</b></div></div><h3>재무상태 요약</h3><div class="report-table"><div><span>자산</span><b>${won(x.assets)}</b></div><div><span>부채</span><b>${won(x.liabilities)}</b></div><div><span>자본(당기순이익 포함)</span><b>${won(displayedEquity)}</b></div></div><h3>자금 현황</h3><div class="report-table"><div><span>보통예금</span><b>${won(bank?(b['code:'+bank.code]??b[bank.id]??0):0)}</b></div><div><span>현금</span><b>${won(cash?(b['code:'+cash.code]??b[cash.id]??0):0)}</b></div><div><span>카드미수금</span><b>${won(recv?(b['code:'+recv.code]??b[recv.id]??0):0)}</b></div><div><span>카드미지급금</span><b>${won(pay?(b['code:'+pay.code]??b[pay.id]??0):0)}</b></div></div><h3>지출 카테고리 TOP 5</h3>${categoryBars(cats)}<h3>자주 기록한 거래처</h3><div class="report-table">${topCp.length?topCp.map(([n,c])=>`<div><span>${esc(n)}</span><b>${c}건</b></div>`).join(''):'<div><span>거래처 기록</span><b>없음</b></div>'}</div><h3>거래 및 증빙</h3><div class="report-table"><div><span>전체 거래</span><b>${total}건</b></div><div><span>증빙 첨부</span><b>${ev}건</b></div><div><span>증빙 미첨부</span><b>${Math.max(0,total-ev)}건</b></div><div><span>증빙 첨부율</span><b>${total?Math.round(ev/total*100):0}%</b></div></div><p class="report-note">※ 본 보고서는 학원 내부 경영관리용입니다. 세무 신고 및 외부 제출용 공식 재무제표는 전문가 검토가 필요합니다.</p></article>`)
}
function settings(){
  const email=state.session.user.email;
  $('#app').innerHTML=shell(`<div class="section-title">설정</div>
  <div class="card"><div class="row"><span>학원</span><b>${state.academy.name}</b></div><div class="row"><span>로그인</span><b>${email}</b></div><div class="row"><span>통화</span><b>KRW (원)</b></div><div class="row"><span>버전</span><b>v9.4.8 카드매출 정산</b></div></div>
