...
function evidencePasteBox(key){
  return `<div class="paste-evidence" id="${key}PasteZone" tabindex="0" role="button" aria-label="증빙 화면 캡처 붙여넣기" onclick="this.focus()" onpaste="handleEvidencePaste(event,'${key}')" ondragover="event.preventDefault();this.classList.add('drag')" ondragleave="this.classList.remove('drag')" ondrop="handleEvidenceDrop(event,'${key}')">
    <div class="paste-evidence-empty" id="${key}PasteEmpty"><b>📋 화면 캡처를 여기에 붙여넣기</b><span>캡처 이미지를 복사한 뒤 이 영역을 클릭하고 <strong>Ctrl + V</strong></span><small>휴대폰은 이미지를 복사한 뒤 이 영역을 길게 눌러 붙여넣기</small></div>
