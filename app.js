...
if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=93').catch(()=>{});
...
function shell(body){
  ...
  return `<div class="app-shell"><aside class="sidebar"><div class="logo-box"><img src="./logo-transparent.png?v=93" alt="수이음 로고"><div class="side-academy">${esc(state.academy?.name||'수이음학원')}</div><div class="side-sub">학원 재무를 한눈에</div></div><nav class="side-nav">${side}</nav><button class="side-logout" onclick="logout()">↪ 로그아웃</button></aside><main class="main"><div class="top"><div class="mobile-brand"><img src="./logo-transparent.png?v=93" alt="수이음"><div><div class="brand">${esc(state.academy?.name||'수이음학원')}</div><div class="muted">학원 재무를 한눈에</div></div></div><button class="btn logout" onclick="logout()">로그아웃</button></div><div class="wrap">${body}</div></main></div><div class="nav"><div class="nav-inner">${bottom}</div></div>`
}
...
function trendSvg(data){
  const vals=data.flatMap(d=>[Number(d.revenue||0),Number(d.expense||0),Math.abs(Number(d.net_income||0))]),max=Math.max(1,...vals),W=680,H=220,p=30;
  const pts=(key)=>data.map((d,i)=>`${p+i*(W-2*p)/Math.max(1,data.length-1)},${H-p-(Number(d[key]||0)/max)*(H-2*p)}`).join(' ');
  const labels=data.map((d,i)=>`<text x="${p+i*(W-2*p)/Math.max(1,data.length-1)}" y="${H-6}" text-anchor="middle">${d.label}</text>`).join('');
  return `<svg class="trend" viewBox="0 0 ${W} ${H}" role="img" aria-label="월별 손익 추이"><line x1="${p}" y1="${H-p}" x2="${W-p}" y2="${H-p}"/><polyline class="rev" points="${pts('revenue')}"/><polyline class="exp" points="${pts('expense')}"/><polyline class="net" points="${pts('net_income')}"/>${labels}</svg>`;
}
async function home(){
  const x=await summary(),[s,e]=monthRange();
  const[{data:tx},hist,b]=await Promise.all([
    sb.from('transactions').select('id,transaction_date,description,evidence_name,transaction_lines(debit,credit,accounts(name,type))').eq('academy_id',state.academy.id).gte('transaction_date',s).lte('transaction_date',e).order('transaction_date',{ascending:false}).limit(8),
    recentSummaries(6),balances()
  ]);
  const total=(tx||[]).length, ev=(tx||[]).filter(t=>t.evidence_name).length, rate=total?Math.round(ev/total*100):0;
  const key=['102','101','103','203'].map(accByCode).filter(Boolean);
  ...
  <div class="dash-grid"><section class="card chart-card"><h3>월별 손익 추이</h3><div class="legend"><span class="blue">● 매출</span><span class="red">● 비용</span><span class="purple">● 순이익</span></div>${trendSvg(hist)}</section><section class="card"><h3>계좌별 잔액</h3>${key.map(a=>`<div class="row"><span>${a.name}</span><b class="${a.code==='203'?'red':''}">${won(b[a.id]||0)}</b></div>`).join('')}<button class="text-link" onclick="go('funds')">자금 현황 전체보기 →</button></section></div>
  <div class="dash-grid three"><section class="card"><h3>이번 달 거래 현황</h3><div class="row"><span>전체 거래</span><b>${total}건</b></div><div class="row"><span>매출 거래</span><b class="blue">${(tx||[]).filter(t=>txKind(t)==='매출').length}건</b></div><div class="row"><span>비용 거래</span><b class="red">${(tx||[]).filter(t=>txKind(t)==='비용').length}건</b></div></section><section class="card evidence-card"><h3>증빙 현황</h3><div class="evidence-rate">${rate}%</div><div class="progress"><i style="width:${rate}%"></i></div><div class="muted">첨부 ${ev}건 / 최근 거래 ${total}건</div><button class="text-link" onclick="evidenceManager()">증빙 관리 →</button></section><section class="card report-card"><h3>월간 보고서</h3><p class="muted">현재 월의 손익·재무상태·자금 현황을 한 장으로 출력합니다.</p><button class="btn primary" onclick="go('report')">보고서 보기 / PDF</button></section></div>
  <div class="section-title">최근 거래</div><div class="card">${tx?.length?tx.map(t=>`<button class="row txrow" onclick="transactionDetail('${t.id}')"><div><b>${esc(t.description)}</b><div class="muted">${t.transaction_date} · ${txKind(t)} ${t.evidence_name?'· 📎 증빙':''}</div></div><span class="amount ${txKindClass(t)}">${won(txAmount(t))}</span></button>`).join(''):'<div class="empty">등록된 거래가 없습니다.<br><button class="text-link" onclick="go(\'add\')">첫 거래 입력하기</button></div>'}</div>${state.install?'<button class="btn install" style="width:100%" onclick="installApp()">📲 홈 화면에 앱 설치하기</button>':''}`;
  $('#app').innerHTML=shell(body)
}

async function transactions(){
  const[s,e]=monthRange();
  const{data}=await sb.from('transactions').select('id,transaction_date,description,evidence_name,transaction_lines(account_id,debit,credit,accounts(id,name,type))').eq('academy_id',state.academy.id).gte('transaction_date',s).lte('transaction_date',e).order('transaction_date',{ascending:false}).limit(500);
  state.txCache=data||[];
  const controls=`<div class="tx-tools"><div class="search-box"><span>⌕</span><input id="txSearch" placeholder="내용 검색" oninput="filterTransactions()"></div><select id="txType" onchange="filterTransactions()"><option value="all">전체 유형</option><option value="매출">매출</option><option value="비용">비용</option><option value="자금이동">자금이동</option><option value="카드대금">카드대금</option></select><select id="txEvidence" onchange="filterTransactions()"><option value="all">증빙 전체</option><option value="yes">증빙 있음</option><option value="no">증빙 없음</option></select></div>`;
...
function filterTransactions(){
  ...
  const list=(state.txCache||[]).filter(t=>{
    const mq=!q||String(t.description||'').toLowerCase().includes(q);
...
async function funds(){const b=await balances();const key=['101','102','103','203'].map(accByCode).filter(Boolean);$('#app').innerHTML=shell(`${monthBar()}<div class="section-title">${monthTitle()} 말 자금 현황</div><div class="grid two">${key.map(a=>`<button class="card metric account-card" onclick="accountDetail('${a.id}')"><span class="muted">${a.name}</span><b class="${a.code==='203'?'red':''}">${won(b[a.id]||0)}</b><small>상세내역 보기 →</small></button>`).join('')}</div><div class="section-title">카드·자금 처리</div><div class="card action-list"><button class="action" onclick="fundAction('cardDeposit')"><b>카드매출 입금</b><span class="muted">카드미수금 → 보통예금</span></button><button class="action" onclick="fundAction('cardPay')"><b>카드대금 결제</b><span class="muted">보통예금 → 카드미지급금</span></button><button class="action" onclick="fundAction('transfer')"><b>현금·계좌 자금이동</b><span class="muted">자산 계정끼리 이동</span></button></div><div class="notice">카드로 매출을 받으면 카드미수금이 늘고, 실제 카드사 입금일에 ‘카드매출 입금’을 기록하세요.</div>`)}
...
async function evidenceManager(){
  const[s,e]=monthRange();
  const{data,error}=await sb.from('transactions').select('id,transaction_date,description,evidence_name,transaction_lines(debit,credit,accounts(name,type))').eq('academy_id',state.academy.id).gte('transaction_date',s).lte('transaction_date',e).order('transaction_date',{ascending:false}).limit(500);
  if(error){alert(error.message);return}
  const list=data||[], attached=list.filter(t=>t.evidence_name), missing=list.filter(t=>!t.evidence_name);
...
function showEvidenceList(mode){
  ...
  el.innerHTML=`<div class="card">${list.length?list.map(t=>`<button class="row txrow" onclick="transactionDetail('${t.id}')"><div><b>${esc(t.description)}</b><div class="muted">${t.transaction_date} · ${txKind(t)} ${t.evidence_name?'· 📎 '+esc(t.evidence_name):'· 증빙 필요'}</div></div><span class="amount ${txKindClass(t)}">${won(txAmount(t))}</span></button>`).join(''):`<div class="empty">${mode==='missing'?'모든 거래에 증빙이 첨부되어 있습니다.':'첨부된 증빙이 없습니다.'}</div>`}</div>`;
}

async function report(){
  const x=await summary(),b=await balances(),[s,e]=monthRange();
  const{data:tx}=await sb.from('transactions').select('id,transaction_date,description,evidence_name,transaction_lines(debit,credit,accounts(name,type))').eq('academy_id',state.academy.id).gte('transaction_date',s).lte('transaction_date',e).order('transaction_date',{ascending:false}).limit(200);
  ...
  const cash=accByCode('101'),bank=accByCode('102'),recv=accByCode('103'),pay=accByCode('203');
  $('#app').innerHTML=shell(`${monthBar()}<div class="report-toolbar"><div><h2>${monthTitle()} 월간 재무 보고서</h2><div class="muted">${esc(state.academy.name)} · 경영관리용</div></div><button class="btn primary" onclick="window.print()">PDF로 저장 / 인쇄</button></div><article class="print-report"><div class="report-brand"><img src="./logo-transparent.png?v=93"><div><h1>${esc(state.academy.name)}</h1><p>${monthTitle()} 월간 재무 보고서</p></div></div><div class="report-kpis"><div><span>매출</span><b>${won(x.revenue)}</b></div><div><span>비용</span><b>${won(x.expense)}</b></div><div><span>순이익</span><b>${won(x.net_income)}</b></div><div><span>자산</span><b>${won(x.assets)}</b></div><div><span>순이익률</span><b>${margin}%</b></div></div><h3>재무상태 요약</h3><div class="report-table"><div><span>자산</span><b>${won(x.assets)}</b></div><div><span>부채</span><b>${won(x.liabilities)}</b></div><div><span>자본(당기순이익 포함)</span><b>${won(displayedEquity)}</b></div></div><h3>자금 현황</h3><div class="report-table"><div><span>보통예금</span><b>${won(bank?b[bank.id]:0)}</b></div><div><span>현금</span><b>${won(cash?b[cash.id]:0)}</b></div><div><span>카드미수금</span><b>${won(recv?b[recv.id]:0)}</b></div><div><span>카드미지급금</span><b>${won(pay?b[pay.id]:0)}</b></div></div><h3>거래 및 증빙</h3><div class="report-table"><div><span>전체 거래</span><b>${total}건</b></div><div><span>증빙 첨부</span><b>${ev}건</b></div><div><span>증빙 첨부율</span><b>${total?Math.round(ev/total*100):0}%</b></div></div><p class="report-note">※ 본 보고서는 학원 내부 경영관리용입니다. 세무 신고 및 외부 제출용 공식 재무제표는 전문가 검토가 필요합니다.</p></article>`)
}

function settings(){const email=state.session.user.email;$('#app').innerHTML=shell(`<div class="section-title">설정</div><div class="card"><div class="row"><span>학원</span><b>${state.academy.name}</b></div><div class="row"><span>로그인</span><b>${email}</b></div><div class="row"><span>통화</span><b>KRW (원)</b></div><div class="row"><span>버전</span><b>v9.3</b></div></div><div class="card"><h3>기본 계정과목</h3>${state.accounts.map(a=>`<div class="row"><span>${a.code} · ${a.name}</span><span class="muted">${a.type}</span></div>`).join('')}</div><div class="notice">증빙사진은 현재 MVP 방식으로 압축해 거래에 저장합니다. 대량 사용 전에는 전용 파일 저장소로 이전하는 것을 권장합니다.</div><div class="notice">이 앱은 경영관리용입니다. 세무 신고·외부감사 목적의 공식 재무제표는 세무/회계 전문가의 검토가 필요합니다.</div>`)}
