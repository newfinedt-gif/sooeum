...
function evidencePasteBox(key){
  return `<div class="paste-evidence" id="${key}PasteZone" role="group" aria-label="증빙 이미지 첨부" ondragover="event.preventDefault();this.classList.add('drag')" ondragleave="this.classList.remove('drag')" ondrop="handleEvidenceDrop(event,'${key}')">
    <div class="paste-evidence-empty" id="${key}PasteEmpty">
      <b>📎 증빙 이미지 넣기</b>
      <div class="drop-primary" onclick="document.getElementById('${key}File').click()">
        <strong>캡처 이미지를 이곳에 끌어다 놓으세요</strong>
        <span>또는 여기를 눌러 이미지 선택</span>
      </div>
      <input id="${key}File" type="file" accept="image/*" hidden onchange="handleEvidenceFile(event,'${key}')">
