...
function evidencePasteBox(key){
  return `<div class="paste-evidence" id="${key}PasteZone" role="group" aria-label="증빙 화면 캡처 붙여넣기" ondragover="event.preventDefault();this.classList.add('drag')" ondragleave="this.classList.remove('drag')" ondrop="handleEvidenceDrop(event,'${key}')">
    <div class="paste-evidence-empty" id="${key}PasteEmpty">
      <b>📋 화면 캡처를 바로 붙여넣기</b>
      <div class="paste-target" id="${key}PasteTarget" contenteditable="true" spellcheck="false" role="textbox" tabindex="0" aria-label="여기를 클릭하고 Ctrl V로 화면 캡처 붙여넣기" onpaste="handleEvidencePaste(event,'${key}')" onkeydown="handlePasteTargetKey(event)">여기를 한 번 클릭한 뒤 <strong>Ctrl + V</strong></div>
...
async function handleEvidencePaste(event,key){
  const items=Array.from(event.clipboardData?.items||[]);
  const item=items.find(x=>x.type&&x.type.startsWith('image/'));
  if(!item){evidenceMessage(key,'이미지가 클립보드에 없습니다. 화면 캡처를 복사한 뒤 다시 붙여넣어 주세요.',true);return}
  event.preventDefault();
  const file=item.getAsFile();
  if(!file){evidenceMessage(key,'붙여넣은 이미지를 읽지 못했습니다.',true);return}
  await setEvidencePaste(key,file);
